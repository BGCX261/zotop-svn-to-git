...
function go(mainUrl,sideUrl){
	if(sideUrl){
		zotop.window.side().location.href = sideUrl;
	}
	if(mainUrl){
		zotop.window.main().location.href = mainUrl;
	}
}
...
function refresh(event) {
	...
	if(keycode == 116 || (event.ctrlKey && keycode==82)) {
		zotop.window.main().location.reload();
		if(document.all) {
			event.keyCode = 0;
...
function msg(){
	var msgUrl = zotop.url.msgUnread;
	if(msgUrl){
...
(function(){
	window.onload=window.onresize=function(){
		...
		//主框架的高度
			...
			$('#body td').css('height',height);
		//设置side部分的高度	
			height = height-$('#side div.header').height()-$('#side div.footer').height();
			$('#side div.body').css('height',height);
	};
})();
...
$(function(){
	$('#mainIframe').load(function(){
		zotop.cookie.set('mainUrl',zotop.window.main().location.href);
	});
})
//给iframe赋值
$(function(){
