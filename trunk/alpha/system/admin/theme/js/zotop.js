	...
	boot:function(){
		...
		$(function(){
			$("body").mouseover(function(){
				window.status = zotop.html.clean(document.title);					 
			});			
		});
...
zotop.dialog = {
	show:function(options){
		...
		switch (settings.type){
			case 'html':
				...
				break;
			case 'iframe':
				//settings.url = zotop.url.join(settings.url,'dialog='+settings.id);
		...
		if(settings.timer && settings.timer>0){
			 ...
			 $dialog.find('.zotop-dialog-timer').timer({second:settings.timer});
		};
		//设置宽高和位置
	...
	tip:function(id,tip){
		if(id){
			$dialog=zotop.dialog.get(id);
			$dialog.find('.zotop-dialog-footer').css('display','block');
			$dialog.find(".zotop-dialog-tip").html('<span class="zotop-tip"> </span>'+tip);
			return $dialog;
		}
		return false;
	},
