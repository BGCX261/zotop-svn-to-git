...
form::header();
	...
	foreach($rows as $row)
	{
		...
		$column['title'] = '<span class="zotop-icon zotop-icon-folder"></span><input name="id[]" type="hidden" value="'.$row['id'].'"/><a href="'.zotop::url('system/folder/index/'.$row['id']).'"><b>'.$row['title'].'</b></a><h5>'.$row['description'].'</h5>';
		$column['manage edit'] = '<a href="'.zotop::url('system/folder/edit/'.$row['id']).'" id="folder'.$row['id'].'" class="dialog">修改</a>';
		$column['manage delete'] = '<a href="'.zotop::url('system/folder/delete/'.$row['id']).'" class="confirm">删除</a>';
		$column['extra w10'] = '';
		table::row($column);
	}
	table::footer();
