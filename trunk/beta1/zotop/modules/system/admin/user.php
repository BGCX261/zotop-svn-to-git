<?php
class system_controller_user extends controller
{
    public function navbar()
    {
        return array(
			array('id'=>'index','title'=>'用户列表','href'=>zotop::url('system/user/index')),
			array('id'=>'add','title'=>'添加新用户','href'=>zotop::url('system/user/add')),
		);
    }
    ...
    public function actionLock($id,$status=-1)
    {
        ...
        if( $update )
        {
            msg::success('操作成功，正在刷新页面，请稍后……',zotop::url('system/user'));   
        }        
    }
	...
	public function actionAdd()
	{
		...
		if(form::isPostBack())
		{
		    ...
		    if( $insert )
		    {
		        msg::success('保存成功，正在返回列表页面，请稍后……',zotop::url('system/user'));   
		    }		
		}
		//获取用户组
	...
	public function actionDelete($id)
    {
		...
		zotop::run('system.user.delete',$user);

		if( $user->delete() )
		{
			msg::success('删除成功，正在重载数据，请稍后……',zotop::url('system/user'));
		}
    }
