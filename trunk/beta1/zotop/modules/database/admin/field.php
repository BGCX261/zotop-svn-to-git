...
class database_controller_field extends controller
{
	...
	{
		return array(
		  'tables'=>array('id'=>'tables','title'=>'数据表管理','href'=>url::build('database/table')),
		  'index'=>array('id'=>'index','title'=>'数据表结构','href'=>url::build('database/field/index/'.$tablename)),
		  'add'=>array('id'=>'add','title'=>'新建字段','href'=>url::build('database/field/add/'.$tablename),'class'=>'dialog'),
		  'edit'=>array('id'=>'edit','title'=>'字段修改','href'=>''),
	   );
	}
    ...
    public function actionIndex($tablename)
    {
        ...
        $page = new page();
        $page->title = '数据库管理';
        $page->set('position','<a href="'.zotop::url('database/table').'">'.$database['database'].' @ '.$database['hostname'].'</a> <em>></em> 数据表 : <b>'.$tablename. '</b>');         
	...
	public function actionAdd($tablename)
	{
	    ...
	    if(form::isPostBack())
		{
			...
			if($result)
			{
				msg::success('字段创建成功',zotop::url('database/field/index/'.$tablename));
			}
	...
	public function actionPrimaryKey($tablename , $fieldname)
	{
		...
		if($result)
		{
			msg::success('已经成功的将该字段设置成为主键',zotop::url('database/field/index/'.$tablename));
		}
	}
	...
	public function actionAddIndex($tablename , $fieldname)
	{
		...
		if($result)
		{
			msg::success('操作成功，已经成功的索引该字段',zotop::url('database/field/index/'.$tablename));
		}
	}
	...
	public function actionDropIndex($tablename , $indexname)
	{
		...
		if($result)
		{
			msg::success('操作成功，已经成功的删除了索引',zotop::url('database/field/index/'.$tablename));
		}
	}
	...
	public function actionUnique($tablename , $fieldname)
	{
		...
		if($result)
		{
			msg::success('操作成功，已经成功的将该字段设置为唯一索引',zotop::url('database/field/index/'.$tablename));
		}
	}
	...
	public function actionFulltext($tablename , $fieldname)
	{
		...
		if($result)
		{
			msg::success('操作成功，已经成功的将该字段设置为全文索引',zotop::url('database/field/index/'.$tablename));
		}
	}
