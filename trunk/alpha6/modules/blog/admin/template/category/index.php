...
form::header();
	...
	foreach($dataset['data'] as $row)
	{
		...
		$column['manage edit'] = '<a href="'.zotop::url('blog/category/edit',array('id'=>$row['id'])).'" id="category'.$row['id'].'" class="dialog">修改</a>';
		$column['manage delete'] = '<a href="'.zotop::url('blog/category/delete',array('id'=>$row['id'])).'" class="confirm">删除</a>';
		$column['extra w10'] = '';
		table::row($column);
	}
	table::footer();
